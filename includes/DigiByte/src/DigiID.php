...
class DigiID {
	...
	/**
     * Extract secret from digiid url
     * @param $uri
     * @return string
     */
	public function extractSecret($uri) {
		if(preg_match('/(&|\?)secret=([^&]+)/', $uri . '&', $match)) {
            return $match[2];
        }
        return '';
	}
    ...
    public function buildURI($callback, $nonce = null) {
        ...
        return $this->_scheme . '://' . $this->_callback . '?x=' . $this->_nonce;
    }
    ...
    public function isAddressValid($address) {
        try {
            $address = $this->base58check_decode($address);
     ...
     * @throws InvalidArgumentException
     */
    public function base58check_decode($str) {
